...
/**
 * Humanizada Page - Galeria de Projetos de Plantas Humanizadas da Pixel Obra
 * Design: Glass Morphism Arquitetônico com tema escuro
 */

// Imagens do portfólio (usando as imagens já geradas e a nova imagem)
const HUMANIZADA_IMAGES = {
  ...
  esboco: "https://d2xsxph8kpxj0f.cloudfront.net/310519663329410379/6BCWYzW7mGqgAm22TjZhXb/plantahmeb_e50c8dca.jpg",
...
const PROJECT_IMAGES: Record<number, string> = {
  1: HUMANIZADA_IMAGES.ideia3,
  2: HUMANIZADA_IMAGES.ideia2,
  3: HUMANIZADA_IMAGES.ideia1,
  8: HUMANIZADA_IMAGES.esboco,
  4: HUMANIZADA_IMAGES.projeto2,
  5: HUMANIZADA_IMAGES.projeto3,
  6: HUMANIZADA_IMAGES.projeto1,
  7: HUMANIZADA_IMAGES.logopo,
};
...
export default function Humanizada() {
  const { language } = useLanguage();
  const t = translations[language].humanizada;
  ...
  const projectGroups = [
    ...
    filteredProjects.slice(4, 7),
    filteredProjects.slice(7),
  ];
  ...
  const renderProjectCard = (project: any, globalIndex: number) => (
    <motion.div
      key={project.id}
      layout
      variants={fadeInUp}
      className="group cursor-pointer"
      onClick={() => openLightbox(project, globalIndex)}
    >
      <div
          ...
          >
            <AnimatePresence mode="popLayout">
              {projectGroups[1].map((project, index) =>
                renderProjectCard(project, 3 + index)
              )}
            </AnimatePresence>
          </motion.div>
